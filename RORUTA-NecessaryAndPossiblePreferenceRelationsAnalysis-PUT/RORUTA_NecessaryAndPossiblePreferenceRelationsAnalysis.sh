#! /bin/bash
# Usage:
#  RORUTA_NecessaryAndPossiblePreferenceRelationsAnalysis.sh [--v2|--v3] input_dir output_dir
...
if [ ! $# == 3 ]; then
  ...
  R --slave --vanilla --file=src/RORUTA_NecessaryAndPossiblePreferenceRelationsAnalysisCLI_XMCDAv3.R --args $2 $3
  ret=$?
elif [ $1 = "--v2" ]; then
  R --slave --vanilla --file=src/RORUTA_NecessaryAndPossiblePreferenceRelationsAnalysisCLI_XMCDAv2.R --args $2 $3
  ret=$?
else
  echo "Usage: $0 [--v2|--v3] input_dir output_dir" >&2
  exit 4
