# usage:
# R --slave --vanilla --file=RORUTA_NecessaryAndPossiblePreferenceRelationsAnalysisCLI_XMCDAv2.R --args "[inDirectory]" "[outDirectory]"
...
source("RORUTA_NecessaryAndPossiblePreferenceRelationsAnalysis.R")
...
#inDirectory <- "/path/to/RORUTA_NecessaryAndPossiblePreferenceRelationsAnalysis/tests/in1.v2"
#outDirectory <- "/path/to/RORUTA_NecessaryAndPossiblePreferenceRelationsAnalysis/tests/out_tmp/"

# filenames

criteriaFile <- "criteria.xml"
characteristicPointsFile <- "characteristicPoints.xml"
criteriaPreferenceDirectionsFile <- "criteriaPreferenceDirections.xml"
...
intensitiesOfPreferencesFile <- "intensityOfPreferences.xml"
rankRelatedRequirementsFile <- "rankRelatedRequirements.xml"
necessaryRelationsFile <- "necessaryRelations.xml"
possibleRelationsFile <- "possibleRelations.xml"
...
loadXMCDAv3(xmcdaData, inDirectory, characteristicPointsFile, mandatory = FALSE, xmcdaMessages, "criteriaValues")
loadXMCDAv3(xmcdaData, inDirectory, criteriaPreferenceDirectionsFile, mandatory = TRUE, xmcdaMessages, "criteriaScales")
...
loadXMCDAv3(xmcdaData, inDirectory, intensitiesOfPreferencesFile, mandatory = FALSE, xmcdaMessages, "alternativesMatrix")
loadXMCDAv3(xmcdaData, inDirectory, rankRelatedRequirementsFile, mandatory = FALSE, xmcdaMessages, "alternativesValues")
...
results <- handleException(
  function() return(
    RORUTA_NecessaryAndPossiblePreferenceRelationsAnalysis(inputs)
  ),
  xmcdaMessages,
...
if (is.null(results)){
  writeXMCDA(xmcdaMessages, paste(outDirectory,messagesFile, sep="/"))
  stop("Could not calculate RORUTA_NecessaryAndPossiblePreferenceRelationsAnalysis.")
}
...
if (is.null(xResults)){
  writeXMCDA(xmcdaMessages, paste(outDirectory,messagesFile, sep="/"))
  stop("Could not convert RORUTA_NecessaryAndPossiblePreferenceRelationsAnalysis results into XMCDA")
}
