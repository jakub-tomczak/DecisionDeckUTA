...
processPreferences <- function(preferencesMatrix, alternatives) {
  ...
  for(row in alternatives)
  {
    ...
    {
      if(!is.null(preferencesMatrix$get(row, column)))
      {
        value <- preferencesMatrix$get(row, column)
        print("ok")
      }
    }
  }
...
checkAndExtractInputs <- function(xmcdaData, programExecutionResult) {
  ...
  preference <- xmcdaData$alternativesMatricesList$get(as.integer(0))
  for(row in alternatives$alternatives)
  {
    for(column in alternatives$alternatives)
    {
      if(!is.null(preference$get(row, column)))
      {
        value <- preference$get(row, column)
        print("ok")
      }
    }
  }
