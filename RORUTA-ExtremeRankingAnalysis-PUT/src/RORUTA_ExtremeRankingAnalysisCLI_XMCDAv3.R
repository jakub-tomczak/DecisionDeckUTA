# usage:
# R --slave --vanilla --file=RORUTA_ExtremeRankingAnalysisCLI_XMCDAv2.R --args "[inDirectory]" "[outDirectory]"
...
source("RORUTA_ExtremeRankingAnalysis.R")
...
#inDirectory <- "/path/to/RORUTA_ExtremeRankingAnalysis/tests/in1.v2"
#outDirectory <- "/path/to/RORUTA_ExtremeRankingAnalysis/tests/out_tmp/"
...
best-rankingFile <- "best-ranking.xml"
worst-rankingFile <- "worst-ranking.xml"
...
results <- handleException(
  function() return(
    RORUTA_ExtremeRankingAnalysis(inputs)
  ),
  xmcdaMessages,
...
if (is.null(results)){
  writeXMCDA(xmcdaMessages, paste(outDirectory,messagesFile, sep="/"))
  stop("Could not calculate RORUTA_ExtremeRankingAnalysis.")
}
...
if (is.null(xResults)){
  writeXMCDA(xmcdaMessages, paste(outDirectory,messagesFile, sep="/"))
  stop("Could not convert RORUTA_ExtremeRankingAnalysis results into XMCDA")
}
