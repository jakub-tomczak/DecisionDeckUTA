#! /bin/bash
# Usage:
#  RORUTA_ExtremeRankingAnalysis.sh [--v2|--v3] input_dir output_dir
...
if [ ! $# == 3 ]; then
  ...
  R --slave --vanilla --file=src/RORUTA_ExtremeRankingAnalysisCLI_XMCDAv3.R --args $2 $3
  ret=$?
elif [ $1 = "--v2" ]; then
  R --slave --vanilla --file=src/RORUTA_ExtremeRankingAnalysisCLI_XMCDAv2.R --args $2 $3
  ret=$?
else
  echo "Usage: $0 [--v2|--v3] input_dir output_dir" >&2
  exit 4
